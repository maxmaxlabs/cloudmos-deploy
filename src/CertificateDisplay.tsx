...
export function CertificateDisplay(props) {
  ...
  const loadValidCertificates = useCallback(async () => {
    ...
    const response = await fetch(
      apiEndpoint +
        "/akash/cert/v1beta1/certificates/list?filter.state=valid&filter.owner=" +
        address
    );
  ...
  async function createCertificate() {
    ...
    var pubpem = rs.KEYUTIL.getPEM(pub, "PKCS8PUB").replaceAll(
      "PUBLIC KEY",
      "EC PUBLIC KEY"
    );
      ...
      ext: [
        {
          extname: "keyUsage",
          critical: true,
          names: ["keyEncipherment", "dataEncipherment"],
        },
        {
          extname: "extKeyUsage",
